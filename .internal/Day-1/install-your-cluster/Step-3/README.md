...
> Because the API server runs as a pod and does not share its filesystem with the mastere node by default. Having `hostPath` volumes allows to persist files across pod restarts.

- Where will audit logs be located ?

> In the `/var/log/kubernetes/audit/audit.log` on the master node
...
root@master-0:~$ ETCDCTL_API=3 etcdctl --endpoints=https://127.0.0.1:2379 \
...
command:
  ...
  - command:
    - etcd
    - --initial-cluster-token=etcd-cluster-1 # Force creation of a new cluster
    ...
...
  volumes:
  - hostPath:
      path: /var/lib/etcd-backup # Target the new data-dir on the host
      type: DirectoryOrCreate
    name: etcd-data
...
# For ALL masters, put back the manifests into place
root@master-0:~$ mv /etc/kubernetes/manifests.old /etc/kubernetes/manifests

# After a seconds/minutes, the important pod you accidentally deleted should be back !
training@bastion:~$ kubectl get po
```
