...
> Add a label on each node

* Add a label `topology.kubernetes.io/zone` onto each worker node. Each with a value `a`, `b` or `c`.

```sh
kubectl label node master-0.wescale.internal topology.kubernetes.io/zone=a
kubectl label node master-1.wescale.internal topology.kubernetes.io/zone=b
kubectl label node master-2.wescale.internal topology.kubernetes.io/zone=c
kubectl label node worker-0.wescale.internal topology.kubernetes.io/zone=a
kubectl label node worker-1.wescale.internal topology.kubernetes.io/zone=b
kubectl label node worker-2.wescale.internal topology.kubernetes.io/zone=c
```

*Note that with Kubernetes managed services on cloud providers, each node is able to describe itself, retrieves its zone, then update its own labels.*
