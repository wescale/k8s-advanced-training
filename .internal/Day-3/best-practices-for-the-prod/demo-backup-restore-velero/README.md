...
```sh
helm install minio -n velero oci://registry-1.docker.io/bitnamicharts/minio --create-namespace --set auth.rootUser=admin,auth.rootPassword=Wesc@leTrain1ng,defaultBuckets=velero
```
...
```sh
export POD_NAME=$(kubectl get pods --namespace velero -l "app.kubernetes.io/name=minio" -o jsonpath="{.items[0].metadata.name}")
kubectl -n velero port-forward --address 0.0.0.0 $POD_NAME 9000
```

URL: http://BASTION_URL:9000/minio/velero/

## Install wordpress sample app

```sh
helm install my-release oci://registry-1.docker.io/bitnamicharts/wordpress -n demo-velero --create-namespace \
  --set wordpressUsername=admin \
  --set wordpressPassword=password \
  --set persistence.storageClass=local-static-provisioner \
  --set mariadb.primary.persistence.storageClass=local-static-provisioner

# Retrieve nodePort to access the app
kubectl get svc -n demo-velero wordpress -o jsonpath='{.spec.ports[1].nodePort}'
```

Access the wordpress admin page and authenticate using the helm release credentials : https://lb.k8s-ops-0.wescaletraining.fr:<NODEPORT>/wp-admin


## Back up

Annotate pods that needs their volumes to be backed up
```
kubectl -n demo-velero annotate pod/wordpress-mariadb-0 backup.velero.io/backup-volumes=data
```

Create a backup for any object that matches the app=nginx label selector:

```sh
velero backup create wordpress-backup --selector app.kubernetes.io/instance=wordpress --include-namespaces demo-velero
```
...
```sh
kubectl delete namespace demo-velero
```

To check that the nginx deployment and service are gone, run:

```sh
kubectl get all -n demo-velero
```
